...
public class ServidorCixJulcaService {
    ...
    public PaginaContenidoDTO buscarAudiosRapido(String numeroMovil, int pagina, int tamano) {
        ...
        try {
            ...
            // 4. Filtrar archivos (solo archivos, no directorios, y con tamaño > 3 minutos)
            List<AudioVentaDTO> archivos = new ArrayList<>();

            // Fecha de hoy (cuando se ejecutó el script y se creó la carpeta)
            ...
            for (ChannelSftp.LsEntry entry : entries) {
                ...
                // Crear AudioVentaDTO con toda la información
                AudioVentaDTO audio = AudioVentaDTO.builder()
                    .nombre(nombre)
                    .fechaCreadaRutaServidor(fechaCreadaRuta) // Fecha cuando se creó la carpeta
    ...
    /**
     * Elimina la carpeta de audios de búsqueda rápida para un móvil específico
     * Ruta: /BUSQUEDA/audios/{fecha}/{numeroMovil}/
     */
    public void eliminarCarpetaAudios(String numeroMovil, String fecha) {
        ...
        try {
            ...
            // Comando para eliminar la carpeta completa
            ...
            if (exitStatus == 0) {
                log.info("✅ Carpeta eliminada exitosamente: {}", rutaCarpeta);
            } else {
                log.warn("⚠️ Comando retornó código: {} para ruta: {}", exitStatus, rutaCarpeta);
            }

        } catch (Exception e) {
            log.error("❌ Error eliminando carpeta de audios: {}", e.getMessage(), e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,
                "Error eliminando carpeta de audios: " + e.getMessage());
        } finally {
            if (channelExec != null && channelExec.isConnected()) {
                channelExec.disconnect();
            }
